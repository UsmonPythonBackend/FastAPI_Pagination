...
from schemas import LikesCreateModel, LikesUpdateModel
...
@likes_router.get('/', response_model=Page)
async def get_like():
    all_likes = session.query(Likes).all()
    return jsonable_encoder(paginate(all_likes))

add_pagination(likes_router)
...
@likes_router.post('/')
async def create_likes(post: LikesCreateModel, authorization: AuthJWT = Depends()):
    try:
        authorization.jwt_required()
...
@likes_router.put("/{id}")
async def update_post(id: int, post: LikesUpdateModel, authorization: AuthJWT = Depends()):
    try:
        authorization.jwt_required()
