...
from schemas import FollowersCreateModel, FollowersUpdateModel
from models import Likes, User, Followers
...
@followers_router.get('/', response_model=Page)
async def get_follower():
    all_followers = session.query(Followers).all()
    return jsonable_encoder(paginate(all_followers))


add_pagination(followers_router)
...
async def create_followers(post: FollowersCreateModel, authorization: AuthJWT = Depends()):
    try:
        authorization.jwt_required()
...
async def update_followers(id: int, post: FollowersUpdateModel, authorization: AuthJWT = Depends()):
    try:
        authorization.jwt_required()
